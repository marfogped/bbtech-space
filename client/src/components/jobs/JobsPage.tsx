...
const JobsPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    callJobs()
  }, [getJobs, jobs, language])

  const filterJobs = (searchValue: string, filters: { [key: string]: boolean }) => {
  ...
  return (
    <section className="w-full h-max overflow-hidden py-5">
      <div className="section-container container-grid">
        ...
        <motion.section layout className='col-span-4 grid xs:grid-cols-2 sm:grid-cols-2 lg:grid-cols-3 xxl:grid-cols-4 gap-5'>
          <AnimatePresence mode='wait'>
            <Suspense fallback="loading">
              {
                (jobs && filteredJobs) && (inputSearch || Object.keys(selectedFilters).length ? filteredJobs : jobs)?.length > 0 ? (
                    (inputSearch || Object.keys(selectedFilters).length ? filteredJobs : jobs).map((job, jobIdx) => (
                      <JobCard 
                        key={job._id} 
                        job={job} 
                        jobIdx={jobIdx}
                  ...
                  <div className='text-3xl font-bold col-span-full flexCenter text-pretty'>
                    {language === "en" && "No matches found"} {language === "es" && "No se encontraron coincidencias"} {language === "it" && "Nessun risultato trovato"}
                  </div>
                )
              }
            </Suspense>
