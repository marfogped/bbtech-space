...
const Privacy: React.FC = () => {
    const [privacy, setPrivacy] = useState<SubSectionTranslation[] | null>(null)
    ...
    useEffect(() => {
        const updateTexts = () => {    
          setTimeout(() => {
            const privacyTranslations: SubSectionTranslation[] = t("subSections", {
              returnObjects: true,
            });
            setPrivacy(privacyTranslations);
        }, 500);
        };
  ...
  return (
    <section className="xs:w-full sm:w-full lg:w-[60%] mx-auto h-max xs:p-8 sm:p-8 xs:py-28 sm:py-28 py-32">
        ...
        {privacy && Array.isArray(privacy)
            ? privacy.map((item, itemIdx) => {
                return (
